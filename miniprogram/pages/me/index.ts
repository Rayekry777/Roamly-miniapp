...
Page({
  data: { user: authStore.user, signs: 0, signed: false, loading: false, loggedIn: authStore.isLoggedIn() },
  onShow() {
    syncTabBar(this)
  ...
  async loadUser() {
    ...
    try {
      const [user, signs] = await Promise.all([refreshCurrentUser(), userService.signCount()])
  ...
  login() { navigateToLogin('/pages/me/index') },
  publish() { if (requireLogin()) wx.navigateTo({ url: '/package-blog/pages/publish/index' }) },
  myBlogs() { if (requireLogin()) wx.navigateTo({ url: '/package-blog/pages/mine/index' }) },
  async logout() {
    try { await signOut() } catch { /* 请求层已经提示网络错误，本地会话仍需退出 */ }
