...
Page({
  ...
  onShow() {
    syncTabBar(this);
    this.setData({ avatar: authStore.user?.icon || "" });
  },
  onUnload() {
    this.scope?.close();
  },
  async loadHome() {
    try {
      const result = await this.scope?.run(
        Promise.all([listShopTypes(), listHotBlogs(1)]),
      );
      if (!result) return;
      ...
      const shopsResult = await this.scope?.run(
        listShopsByType({ typeId: first.id, current: 1, ...location }),
      );
  ...
  getLocationFallback(): Promise<{ x?: number; y?: number }> {
    return new Promise((resolve) =>
      wx.getLocation({
        type: "gcj02",
        timeout: 3000,
        success: (res) => resolve({ x: res.longitude, y: res.latitude }),
        fail: () => resolve({}),
      }),
    );
  },
  openShops(event?: WechatMiniprogram.TouchEvent) {
    ...
    wx.navigateTo({
      url: `/package-shop/pages/list/index${typeId ? `?typeId=${typeId}&name=${encodeURIComponent(name)}` : ""}`,
    });
  },
  openShop(event: WechatMiniprogram.CustomEvent<{ id: string }>) {
    wx.navigateTo({
      url: `/package-shop/pages/detail/index?id=${event.detail.id}`,
    });
  },
  ...
  openSections() {
    navigateToTab("/pages/sections/index");
  },
