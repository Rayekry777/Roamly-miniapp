import { imageError, imageUrl, resolveRenderableImage } from '../../utils/media'
...
Component({
  ...
  observers: { 'src,kind'(src: string, kind: 'photo' | 'avatar' | 'category') { void this.resolveSource?.(src, kind) } },
  methods: {
    async resolveSource(src: string, kind: 'photo' | 'avatar' | 'category') {
      ...
      try {
        ...
        if (version === this.data.resolveVersion) this.setData({ resolved: imageError(kind) })
      }
    },
    onError() { this.setData({ resolved: imageError(this.data.kind as 'photo' | 'avatar' | 'category') }) }
  }
})
