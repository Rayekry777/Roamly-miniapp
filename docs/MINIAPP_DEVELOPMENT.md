...
- 当前首页、五入口导航、统一发布器和用户动态入口均使用 Post 契约；Blog 兼容页面与 API 已退役。
...
- 当前社区首页、用户主页和发布模型为 `Post`，图片先上传为临时媒体并提交字符串 `mediaIds`；旧 Blog 页面已删除。
- 当前 Post 详情、探店商户连接和评论交互界面已经完成；后端 7 个评论接口、热门评论批量摘要和小程序响应适配已经实现，旧 Blog 详情不再提供读取兼容。
...
| `/package-shop/pages/reviews/index`    | 商户点评列表      | 公开，写入登录     | 已实现 |
...
- `package-blog` 已删除，统一由 `package-post` 承载动态详情、发布和我的动态。
...
- 阶段 9 已将商户详情优惠区域切换为目标 `/v1/shops/{shopId}/voucher-products`，商品详情使用 `/v1/voucher-products/{productId}`；后端接口尚未实现时页面展示真实错误和重试，不回退到旧秒杀接口或本地假数据。`/v1/shops/{shopId}/posts` 尚未实现，按需请求失败时展示真实错误和重试；独立点评接口尚未由后端提供，前端阶段 8 已完成契约适配并保留真实错误状态。
...
- `/package-shop/pages/reviews/index` 已实现最新/高分排序、页码分页、字符串 ID 去重、首屏骨架、空状态、错误重试和上拉加载。
- 点评编辑器支持 1～5 分、1～2000 字正文和最多 9 张 JPEG/PNG/WebP 图片；图片沿用临时媒体资产流程，上传中或失败时禁止提交，删除新上传媒体会尝试清理服务端临时资产。
- 客户端请求只提交 `score/content/mediaIds`，不提交 `verifiedConsumption`；“已消费”仅消费后端响应字段。已有点评通过响应中的 `editable` 标识进入编辑，编辑态不会删除已绑定媒体资产。
- 创建返回 `REVIEW_ALREADY_EXISTS` 或 HTTP 409 时保留表单并刷新列表，提示用户从自己的点评进入编辑；401 由会话层清理 Token 并恢复当前点评页。
- 当前后端尚未实现 `/v1/shops/{shopId}/reviews`、`/me` 相关接口和 `shop_review` 表，页面会展示“点评接口尚未完成升级”或服务端真实错误，不回退到本地假数据，也不会虚构消费标识。
...
- 已新增 `VoucherProduct`、`VoucherOrder`、`UserVoucher` 类型及金额、状态和字符串 ID 适配；金额在接口中按分保存，页面仅格式化为元展示。
- 已新增商品、订单和券包 API/Service，以及商品详情、订单列表/详情、券包列表/详情页面；订单数量固定为 1，只有待支付订单显示取消操作。
- 创建订单前执行二次确认；前端不接入微信支付，不把订单创建误显示为支付成功，待支付订单详情明确提示当前支付能力未接入。
- 券码仅在未使用券详情/列表展示，代码、日志和埋点不输出完整券码。
- 当前后端尚未实现 `voucher-products`、新订单和用户券包接口及对应数据表，页面会展示“接口尚未完成升级”或服务端真实错误；阶段 9 前端状态机已完成，真实联调顺延。
...
|    8 | 实现点评发布、编辑和列表                     | 唯一点评、图片和评分刷新通过       | 已实现 |
|    9 | 实现团购详情、订单和券包                     | 状态展示、取消和资产权限通过       | 已实现（前端；后端联调未完成） |
|   10 | 删除 Blog 兼容类型、页面和旧 API             | 后端切换、全量回归和无旧引用       | 已实现（前端；后端旧接口退役待联调） |
...
| 2026-09-02 | 阶段 6：动态详情与 Threads 评论               | `npm run verify` 通过：26 个测试文件、90 项测试；`npm run build:npm` 成功生成 2 个依赖。完成 Post 详情响应适配、探店商户连接、动态点赞和作者关注、热门/最新评论状态隔离、一级缩进回复串、回复分页、底部输入框、登录后输入恢复、评论点赞回滚、删除占位及最多 3 页的评论定位；当时后端评论接口尚未实现，真实联调顺延。                                                                      |
...
| 2026-09-03 | 阶段 8：商户点评发布、编辑和列表              | `npm run verify` 通过：32 个测试文件、112 项测试；完成点评列表排序/分页、空/错误/重试状态、登录门禁、评分和正文边界、9 图临时媒体上传、编辑/删除、重复点评 409 提示及消费标识只读适配。后端点评接口、点评表和评分聚合尚未实现，真实联调顺延。                                                                                                                                                                                                                                       |
| 2026-09-03 | 阶段 9：团购详情、订单和券包                  | `npm run verify` 通过：32 个测试文件、113 项测试；`npm run type-check`、`npm run lint`、`npm run lint:styles` 通过。完成团购商品、订单、用户券类型与 API/Service，新增商品详情、订单列表/详情、券包列表/详情分包页面，支持状态筛选、订单取消、金额展示和真实错误重试；后端目标接口与数据表尚未实现，未进行真实联调。 |
| 2026-09-03 | 评论正式契约对齐 | `npm run verify` 通过：32 个测试文件、114 项测试。API 层新增 `CommentCreateDTO`、`CommentResponse`、`CommentThreadResponse`；Service 层统一适配 `deleted/postAuthor/deletable/previewReplies`、字符串 ID、删除占位和动态上下文 `postId`，组件使用服务端作者标识与删除权限。后端评论 HTTP 和 OpenAPI 已实现，运行数据库重建与 Android/iOS 真机联调待完成。|

| 2026-09-03 | 阶段 10：删除 Blog 兼容类型、页面和旧 API    | `npm run verify` 通过：33 个测试文件、117 项测试；删除 `package-blog`、旧发现/关注页面、Blog API/Service/组件及旧上传入口，用户主页和“我的动态”统一使用 Post；后端旧接口退役、真机联调待完成。 |
